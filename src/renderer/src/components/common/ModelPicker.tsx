...
export function ModelPicker() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <BoxIcon size={15} className="text-oracle-accent" />
      ...
      {open && models.length > 0 && (
        <div className="absolute left-0 top-11 z-30 max-h-[60vh] w-[360px] animate-fade-in overflow-y-auto rounded-xl border border-oracle-border bg-oracle-surface p-1.5 shadow-2xl">
          {models.map((m) => {
            ...
            return (
              <button
                ...
                className={`flex w-full items-center gap-3 rounded-lg px-2.5 py-2 text-left transition-colors ${
                  isLoaded ? 'bg-oracle-surface-2' : 'hover:bg-oracle-surface-2'
                }`}
              >
                <div className="min-w-0 flex-1">
                  <div className="truncate text-[13px] font-medium text-oracle-text">
                    {m.filename.replace(/\.gguf$/i, '')}
                  </div>
                  <div className="flex items-center gap-2 text-[11px] text-oracle-muted">
                    <span className="truncate">{m.repoId}</span>
                    {m.quant && <span className="chip">{m.quant}</span>}
                    <span>{formatBytes(m.sizeBytes)}</span>
                  </div>
                </div>
                {isLoaded && <CheckIcon size={16} className="shrink-0 text-oracle-accent" />}
              </button>
            )
          })}
        </div>
